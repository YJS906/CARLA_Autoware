...
브릿지는 점군을 만들 때 ego 위치와 yaw만 사용한다. 따라서 TF에 차체 roll/pitch가
있더라도 필터는 TF에서 ego 원점을 복원한 뒤 같은 yaw 기준으로 상자를 변환한다.
전체 3차원 TF를 그대로 적용하면 차체 기울기에 따라 제외 대상의 윗면 점이 남을 수 있다.
