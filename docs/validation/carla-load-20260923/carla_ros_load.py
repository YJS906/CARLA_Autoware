...
def cb(topic,msg):
 ...
 rows[topic].append(d)
base={'/clock','/localization/kinematic_state','/perception/object_recognition/tracking/objects','/perception/object_recognition/objects','/planning/scenario_planning/trajectory','/control/command/control_cmd','/control/trajectory_follower/control_cmd','/diagnostics'}
time.sleep(1)
...
t0=time.monotonic()
while time.monotonic()-t0<30:rclpy.spin_once(n,timeout_sec=.05)
def stats(v):
...
open('/tmp/carla-ros-load.json','w').write(json.dumps(result,indent=2));print(json.dumps({'streams':{t:v for t,v in summary.items() if t in base},'non_ok_diagnostic_count':len(diags)}));n.destroy_node();rclpy.shutdown()
