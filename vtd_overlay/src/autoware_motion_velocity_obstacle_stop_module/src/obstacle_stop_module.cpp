  ...
  const std::shared_ptr<const PlannerData> planner_data)
{
  ...
  ego_motion_.reset();
  const double actual_speed = planner_data->current_odometry.twist.twist.linear.x;
  const double actual_acceleration = planner_data->current_acceleration.accel.accel.linear.x;
  const double acceleration_limit = planner_data->velocity_smoother_
                                      ? planner_data->velocity_smoother_->getMaxAccel()
                                      : std::numeric_limits<double>::quiet_NaN();
  if (std::isfinite(actual_speed) && std::isfinite(actual_acceleration) &&
      std::isfinite(acceleration_limit) && acceleration_limit >= 0.0) {
    ego_motion_ = selfcar::trajectory_safety::EgoMotion{
      actual_speed, std::max(acceleration_limit, std::abs(actual_acceleration))};
  }
  ...
  const double decimate_trajectory_step_length) const
{
  if (trajectory_polygon_for_inside_map_.count(polygon_param) == 0) {
    ...
    auto traj_polys = selfcar::trajectory_safety::createReachablePosePolygons(
      ...
      ego_motion_, polygon_param.off_track_scale);
  ...
  const TrajectoryPolygonCollisionCheck & trajectory_polygon_collision_check) const
{
  if (!decimated_traj_polys_) {
    ...
    decimated_traj_polys_ = selfcar::trajectory_safety::createReachablePosePolygons(
      decimated_traj_points, vehicle_info, current_pose, 0.0, p.enable_to_consider_current_pose,
      p.time_to_convergence, p.decimate_trajectory_step_length, ego_motion_);
  }
