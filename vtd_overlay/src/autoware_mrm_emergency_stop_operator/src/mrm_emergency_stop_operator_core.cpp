...
: Node("mrm_emergency_stop_operator", node_options)
{
  ...
  params_.immediate_stop = declare_parameter<bool>("immediate_stop", false);
  ...
  const std::vector<rclcpp::Parameter> & parameters)
{
  ...
  update_param<bool>(parameters, "immediate_stop", params_.immediate_stop);
...
Control MrmEmergencyStopOperator::calcTargetAcceleration(const Control & prev_control_cmd) const
{
  ...
  if (params_.immediate_stop) {
    // Keep steering continuous, but command the full configured braking effort in this cycle.
    if (is_prev_control_cmd_subscribed_) control_cmd = prev_control_cmd;
    ...
    control_cmd.longitudinal.is_defined_acceleration = true;
    control_cmd.longitudinal.jerk = 0.0;
    control_cmd.longitudinal.is_defined_jerk = false;  // Acceleration step, not zero-jerk ramping.
    return control_cmd;
  }
