...
DecisionResult IntersectionModule::modifyPathVelocityDetail(PathWithLaneId * path)
{
  ...
  if (is_permanent_go_) {
    // Collision checking above also runs after commitment. Honor its STOP state, including
    // the existing clear-time hold, regardless of traffic-light presence or merging geometry.
    return makePostPassJudgeDecision(
      has_collision_with_margin,
      has_collision_with_margin && can_smoothly_stop_at(*path, closest_idx, collision_stopline_idx),
