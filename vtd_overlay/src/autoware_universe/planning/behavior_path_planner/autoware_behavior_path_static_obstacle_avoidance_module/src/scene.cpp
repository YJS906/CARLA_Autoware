...
void StaticObstacleAvoidanceModule::updateData()
{
  ...
  const bool no_active_avoidance =
    stored_objects_.empty() && clip_objects_.empty() && path_shifter_.getShiftLines().empty() &&
    std::abs(path_shifter_.getBaseOffset()) <= parameters_->lateral_execution_threshold &&
    left_shift_array_.empty() && right_shift_array_.empty();
  if (no_detected_objects && no_active_avoidance) {
    // Do not rebuild lane bounds, shift lines and safety-check paths when there is nothing to
    // avoid. The full module remains active and resumes normal processing as soon as an object or
    // an in-progress avoidance state exists.
    safe_ = true;
    updateRTCData();
    return;
  }
