...
LCParamPtr LaneChangeModuleManager::set_params(rclcpp::Node * node, const std::string & node_name)
{
  ...
  // trajectory generation
  {
    ...
    p.trajectory.target_lane_backward_overlap_length = get_or_declare_parameter<double>(
      *node, parameter("trajectory.target_lane_backward_overlap_length"));
  ...
  p.enable_direct_multi_lane_change =
    get_or_declare_parameter<bool>(*node, parameter("enable_direct_multi_lane_change"));
...
void LaneChangeModuleManager::updateModuleParams(const std::vector<rclcpp::Parameter> & parameters)
{
...

  {
    ...
    update_param<bool>(
      parameters, ns + "enable_direct_multi_lane_change", p->enable_direct_multi_lane_change);
    update_param<double>(
...

  {
    ...
    update_param<double>(
      parameters, ns + "target_lane_backward_overlap_length",
      p->trajectory.target_lane_backward_overlap_length);
