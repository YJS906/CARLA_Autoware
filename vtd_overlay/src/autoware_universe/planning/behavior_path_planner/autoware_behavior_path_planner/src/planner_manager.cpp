  ...
  const std::shared_ptr<PlannerData> & data, const bool is_any_approved_module_running)
{
  ...
  const auto snap_to_global_route_if_ego_out = [&]() {
    ...
    if (route_handler->getClosestLaneletWithConstrainsWithinRoute(
      ...
      // During autonomous driving, avoid a broad nearest-lane snap. However, after a lane change
      // at a split, the graph-based tracker can remain on the old branch even though the ego is
      // already inside the selected route lane. Updating only in that geometrically unambiguous
      // case keeps the original autonomous-snap protection while recovering the reference path.
      if (!skip_global_route_snap || utils::isInLanelets(pose, {constrained})) {
