...
bool is_vehicle(const autoware_perception_msgs::msg::PredictedObject & object)
{
  constexpr auto is_vehicle_class = [](const auto & c) {
    // VTD reports vehicle actors as UNKNOWN. Include them without changing their classification
    // for other modules; all velocity, range, heading and collision checks still apply below.
