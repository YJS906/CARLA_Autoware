...
std::optional<geometry_msgs::msg::Point> find_closest_collision_point(
  ...
  const autoware_utils::Polygon2d & object_footprint)
{
  ...
  for (const auto & rough_collision : rough_collisions) {
    ...
    const auto angle_diff = autoware_utils::normalize_radian(
      tf2::getYaw(ego_pose.orientation) - tf2::getYaw(object_pose.orientation));
    // Check objects at least 60 degrees from this trajectory sample's direction.
...
std::vector<Collision> find_collisions(
  ...
  const autoware_utils::MultiPolygon2d & object_forward_footprints)
{
  ...
  for (auto object_idx = 0UL; object_idx < objects.size(); ++object_idx) {
    ...
    if (collision) {
      Collision c;
      c.object_uuid = autoware_utils::to_hex_string(objects[object_idx].object_id);
      c.point = *collision;
