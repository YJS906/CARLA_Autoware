...
protected:  // for the static_centerline_generator package
  ...
  bool enable_drivable_area_recovery_{false};
  DrivableAreaRecoveryState drivable_area_recovery_;
  ...
  rclcpp::Subscription<std_msgs::msg::Header>::SharedPtr boundary_reset_sub_;
  std::optional<std_msgs::msg::Header> boundary_reset_request_;
  ...
  std::vector<TrajectoryPoint> optimizeTrajectory(
    const PlannerData & planner_data, bool reset_previous = false);
  ...
  void insertZeroVelocityOutsideDrivableArea(
    ...
    std::optional<size_t> first_outside_idx) const;
  std::optional<size_t> findFirstOutsideDrivableArea(
    const PlannerData & planner_data, const std::vector<TrajectoryPoint> & traj_points) const;
