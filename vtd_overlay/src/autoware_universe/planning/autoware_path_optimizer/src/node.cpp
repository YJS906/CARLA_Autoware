  ...
  conditional_timer_(std::make_shared<ConditionalTimer>())
{
  ...
  boundary_stop_pub_ = create_publisher<autoware_internal_planning_msgs::msg::PlanningFactorArray>(
  ...
  boundary_reset_sub_ = create_subscription<Header>(
    "/planning/path_optimizer/reset_previous", rclcpp::QoS(1).reliable(),
    [this](const Header::ConstSharedPtr message) { boundary_reset_request_ = *message; });
  ...
  {  // parameters
    ...
    enable_drivable_area_recovery_ =
      declare_parameter<bool>("option.enable_drivable_area_recovery", false);
  ...
  const std::vector<rclcpp::Parameter> & parameters)
{
  ...
  update_param<bool>(
    parameters, "option.enable_drivable_area_recovery", enable_drivable_area_recovery_);
  update_param<bool>(
...
{
  drivable_area_recovery_.reset();
  boundary_reset_request_.reset();
  ...
  const PlannerData & planner_data)
{
  ...
  bool reset_for_replacement = false;
  if (boundary_reset_request_) {
    const auto age = (now() - rclcpp::Time(boundary_reset_request_->stamp)).seconds();
    if (age < 0.0 || age > 1.0 || !enable_drivable_area_recovery_) {
      boundary_reset_request_.reset();
    } else if (
      planner_data.header.frame_id == boundary_reset_request_->frame_id &&
      rclcpp::Time(planner_data.header.stamp) >= rclcpp::Time(boundary_reset_request_->stamp) &&
      std::isfinite(planner_data.ego_vel) && std::abs(planner_data.ego_vel) <= 0.1) {
      reset_for_replacement = true;
      boundary_reset_request_.reset();
      drivable_area_recovery_.reset();
      // This request itself is the cold retry; do not run it twice in this callback.
      (void)drivable_area_recovery_.update(now().seconds(), true, true);
      RCLCPP_INFO(get_logger(), "Boundary-stop recovery: optimize replacement from current pose");
    }
  }
  auto optimized_traj_points = optimizeTrajectory(planner_data, reset_for_replacement);

  // A stopped vehicle can otherwise keep receiving the same invalid, fixed previous solution.
  // Retry once without it, before publishing anything. Never remove the boundary stop.
  auto outside = findFirstOutsideDrivableArea(planner_data, optimized_traj_points);
  const bool near_stop = outside && autoware::motion_utils::calcSignedArcLength(
                                      optimized_traj_points, planner_data.ego_pose.position,
                                      optimized_traj_points.at(*outside).pose.position) <= 2.0;
  const bool eligible = enable_drivable_area_recovery_ && enable_outside_drivable_area_stop_ &&
                        !enable_skip_optimization_ && std::isfinite(planner_data.ego_vel) &&
                        std::abs(planner_data.ego_vel) <= 0.1 && near_stop;
  if (drivable_area_recovery_.update(now().seconds(), outside.has_value(), eligible)) {
    optimized_traj_points = optimizeTrajectory(planner_data, true);
    outside = findFirstOutsideDrivableArea(planner_data, optimized_traj_points);
    RCLCPP_WARN(
      get_logger(), "Boundary-stop recovery: cold optimization %s; boundary check retained",
      outside ? "still outside drivable area" : "passed");
  }

  // 2. insert zero velocity when trajectory is over drivable area
  insertZeroVelocityOutsideDrivableArea(planner_data, optimized_traj_points, outside);
...
std::vector<TrajectoryPoint> PathOptimizer::optimizeTrajectory(
  const PlannerData & planner_data, const bool reset_previous)
{
  autoware_utils::ScopedTimeTrack st(__func__, *time_keeper_);
  ...
  const bool is_replan_required = [&]() {
    ...
    if (reset_previous || enable_reset_prev_optimization_ || reset_prev_optimization) {
...
std::optional<size_t> PathOptimizer::findFirstOutsideDrivableArea(
  const PlannerData & planner_data,
  const std::vector<TrajectoryPoint> & optimized_traj_points) const
{
  ...
  if (end_idx <= static_cast<int>(ego_idx)) return std::nullopt;
...
void PathOptimizer::insertZeroVelocityOutsideDrivableArea(
  ...
  const std::optional<size_t> first_outside_idx) const
{
  using Factor = autoware_internal_planning_msgs::msg::PlanningFactor;
  autoware_internal_planning_msgs::msg::PlanningFactorArray feedback;
  feedback.header = planner_data.header;
  ...
  if (first_outside_idx) {
    ...
    if (enable_outside_drivable_area_stop_) {
      ...
      if (enable_drivable_area_recovery_) {
        Factor factor;
        factor.module = "path_optimizer";
        ...
        point.pose = optimized_traj_points.at(stop_idx).pose;
        point.velocity = 0.0;
        point.distance = autoware::motion_utils::calcSignedArcLength(
          optimized_traj_points, planner_data.ego_pose.position, point.pose.position);
        factor.control_points.push_back(point);
        feedback.factors.push_back(factor);
      }
    }
  } else {
    debug_data_ptr_->stop_pose_by_drivable_area = std::nullopt;
  }
  // Empty feedback explicitly clears a resolved stop. The consumer also expires stale data.
  boundary_stop_pub_->publish(feedback);
}
