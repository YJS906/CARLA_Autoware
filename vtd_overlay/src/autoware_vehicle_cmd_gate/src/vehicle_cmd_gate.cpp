...
: Node("vehicle_cmd_gate", node_options), is_engaged_(false), updater_(this)
{
  ...
  immediate_emergency_stop_ = declare_parameter<bool>("immediate_emergency_stop", false);
  ...
  const std::vector<rclcpp::Parameter> & parameters)
{
  ...
  update_param<bool>(parameters, "immediate_emergency_stop", immediate_emergency_stop_);
  update_param<double>(
...
void VehicleCmdGate::publishControlCommands(const Commands & commands)
{
  ...
  // An active emergency must not be weakened by the ordinary pause/disengage hold command.
  // No effect on nominal driving, comfortable-stop MRM, or emergency detection thresholds.
  if (immediate_emergency_stop_ && use_emergency_handling_ && is_system_emergency_) {
    filtered_control.longitudinal.stamp = now();
    filtered_control.longitudinal.velocity = 0.0;
    filtered_control.longitudinal.acceleration = static_cast<float>(emergency_acceleration_);
    filtered_control.longitudinal.is_defined_acceleration = true;
    filtered_control.longitudinal.jerk = 0.0;
    filtered_control.longitudinal.is_defined_jerk = false;
  }
...
Control VehicleCmdGate::filterControlCommand(const Control & in)
{
  ...
  const bool immediate_emergency =
    immediate_emergency_stop_ && use_emergency_handling_ && is_system_emergency_;

  // Apply transition_filter when transiting from MANUAL to AUTO.
  if (mode.is_in_transition) {
    filter_on_transition_.filterAll(
      dt, current_steer_, out, is_filter_activated, !immediate_emergency);
  } else {
    filter_.filterAll(dt, current_steer_, out, is_filter_activated, !immediate_emergency);
  }
  ...
  if (ego_is_stopped || immediate_emergency) {
