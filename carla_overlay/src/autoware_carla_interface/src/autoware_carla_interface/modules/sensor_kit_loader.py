...
from .vehicle_adapter import base_to_actor_sensor_transform, rotation_matrix
...
class SensorKitLoader:
    ...
    def __init__(self, logger: Optional[logging.Logger] = None):
        ...
        self.base_link_origin_carla = None
    ...
    def _create_sensor_config(
        ...
        # Calibration uses ROS base_link axes. LiDAR measurement origin can
        # differ from its mounting joint (the VLP-16 scan frame is 3.77 cm up).
        if transform:
            transform = dict(transform)
            offset = mapping.get("measurement_frame_offset", {})
            if offset:
                rotation = rotation_matrix(*(transform[k] for k in ("roll", "pitch", "yaw")))
                delta = rotation @ [offset.get(k, 0.0) for k in ("x", "y", "z")]
                for axis, value in zip(("x", "y", "z"), delta):
                    transform[axis] += float(value)
    ...
    def _carla_baselink_to_vehicle_center_transform(
        ...
        """
        Convert ROS base_link coordinates to CARLA actor coordinates.

        The CARLA-specific sensor calibration uses ROS axes (Y-left). Apply
        the measured rear-axle offset and the ROS-to-CARLA reflection once.

        Args:
        ----
            baselink_transform: ROS transform dict with angles in radians
        ...
        """
        if self.base_link_origin_carla is None:
            raise RuntimeError("Vehicle rear axle must be measured before loading sensors")
        return base_to_actor_sensor_transform(baselink_transform, self.base_link_origin_carla)
