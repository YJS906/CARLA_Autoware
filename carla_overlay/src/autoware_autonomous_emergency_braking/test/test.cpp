...
TEST_F(TestAEB, predictedPathEmpty)
{
  Trajectory trajectory;
  trajectory.header.frame_id = "base_link";
  EXPECT_FALSE(aeb_node_->generateEgoPath(trajectory).has_value());
}

TEST_F(TestAEB, predictedPathRetainsFirstPoint)
{
  // The origin is a valid first pose. Reading path.back() before inserting it is
  // undefined behavior and can also incorrectly drop this point.
  Trajectory trajectory;
  trajectory.header.frame_id = "base_link";
  trajectory.points.resize(1);
  trajectory.points.front().pose.orientation.w = 1.0;
  const auto path = aeb_node_->generateEgoPath(trajectory);
  ASSERT_TRUE(path.has_value());
  ASSERT_EQ(path->size(), 1U);
  EXPECT_DOUBLE_EQ(path->front().position.x, 0.0);
  EXPECT_DOUBLE_EQ(path->front().position.y, 0.0);
}

TEST_F(TestAEB, predictedPathFiltersDuplicatesAfterFirstPoint)
{
  Trajectory trajectory;
  trajectory.header.frame_id = "base_link";
  for (const double x : {0.0, 0.0, 0.005, 1.0, 1.0, 2.0}) {
  ...
  EXPECT_DOUBLE_EQ(path->at(0).position.x, 0.0);
  EXPECT_DOUBLE_EQ(path->at(1).position.x, 1.0);
  EXPECT_DOUBLE_EQ(path->at(2).position.x, 2.0);
}
