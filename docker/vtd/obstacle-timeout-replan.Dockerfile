FROM selfcar-2026-vtd:backup-20260912-almost-final
...
RUN source /opt/ros/jazzy/setup.bash && source /opt/autoware/setup.bash && source /opt/selfcar_overlay/setup.bash && \
    colcon build --base-paths /opt/selfcar-build/src/selfcar_obstacle_timeout_replan \
      ...
      --build-base /tmp/obstacle-timeout-build --install-base /opt/selfcar_overlay --merge-install \
      --cmake-args -DBUILD_TESTING=OFF && rm -rf /tmp/obstacle-timeout-build
